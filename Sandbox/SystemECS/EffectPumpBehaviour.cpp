...
void EffectPumpBehaviour::update(Entity entity) {
	...
	if (GLFWFunctions::isPumpOn) {
		...
		auto& bubblesTransform = ecsCoordinator.getComponent<TransformComponent>(bubblesEntity);
		...
		CollisionSystemECS::OBB bubblesOBB = collisionSystem.createOBBFromEntity(bubblesEntity);
		...
		if (isColliding) {
			...
			forceManager.AddForce(playerEntity, bubblesDirectionalVec);
